...
export async function createEvent(prevState: FormState, formData: FormData) {
  ...
  const guestImageUrls = JSON.parse(
    formData.get("guestImages") as string
  ) as string[];
  ...
  const collaboratorsImages = JSON.parse(
    formData.get("collaboratorsImages") as string
  ) as string[];
  const sponsors = formData.get("sponsorsName") as string;
  const sponsorsDetails = formData.get("sponsorsDetails") as string;
  const sponsorsImages = JSON.parse(
    formData.get("sponsorsImages") as string
  ) as string[];
